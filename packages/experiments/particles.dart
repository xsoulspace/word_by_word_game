...
class ParticlesExample extends FlameGame {
  ...
  void onMount() {
    ...
    spawnTimer = Timer.periodic(sceneDuration, (final _) {
      spawnParticles();
    });
  }
  ...
  Particle randomMovingParticle() => MovingParticle(
  ...
  Particle alignedMovingParticles() => Particle.generate(
        count: 5,
        generator: (final i) {
  ...
  Particle randomMovingParticles() => Particle.generate(
        count: 5,
        generator: (final i) => MovingParticle(
  ...
  /// extension.
  Particle easedMovingParticle() => Particle.generate(
        count: 5,
        generator: (final i) => MovingParticle(
  ...
  Particle intervalMovingParticle() => Particle.generate(
        count: 5,
        generator: (final i) => MovingParticle(
  ...
  Particle computedParticle() => ComputedParticle(
        renderer: (final canvas, final particle) => canvas.drawCircle(
          Offset.zero,
  ...
  Particle steppedComputedParticle() => ComputedParticle(
        lifespan: 2,
        renderer: (final canvas, final particle) {
          const steps = 5;
  ...
  Particle imageParticle() => ImageParticle(
        size: Vector2.all(24),
  ...
  Particle reuseImageParticle() {
    ...
    return Particle.generate(
      count: count,
      generator: (final i) => TranslatedParticle(
        offset: Vector2(
  ...
  Particle rotatingImage({final double initialAngle = 0}) =>
      RotatingParticle(from: initialAngle, child: imageParticle());
  ...
  Particle acceleratedParticles() => Particle.generate(
  ...
  Particle paintParticle() {
    ...
    return Particle.generate(
      count: 3,
      generator: (final i) => PaintParticle(
  ...
  Particle spriteParticle() => SpriteParticle(
  ...
  Particle animationParticle() => SpriteAnimationParticle(
  ...
  Particle componentParticle() => MovingParticle(
  ...
  Particle fireworkParticle() {
    ...
    final paints = [
      ...
      // to overall effect
      Colors.blue,
    ].map((final color) => Paint()..color = color).toList();
  ...
  Particle chainingBehaviors() {
    ...
    final rect = ComputedParticle(
      renderer: (final canvas, final _) => canvas.drawRect(
  ...
  Vector2 randomCellVector2() =>
      (Vector2.random() - Vector2.random())..multiply(cellSize);
  ...
  T randomElement<T>(final List<T> list) => list[rnd.nextInt(list.length)];
...
class TrafficLightComponent extends Component {
  ...
  Color get currentColor =>
      colors[(colorChangeTimer.progress * colors.length).toInt()];
}
