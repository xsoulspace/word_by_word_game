      ...
      for (final fullPath in paths) {
        ...
        map.update(
          ...
          (final value) => value.copyWith(
            framesLength: value.framesLength + 1,
            framesPaths: [...value.framesPaths, fullPath],
          ),
          ifAbsent: () => AnimationEntryModel.singleFrame(fullPath),
        );
      }
...
class ResourcesComponent extends Component with HasGameRef<GameRenderer> {
  ...
  String get terrainWaterPath =>
      _getAssetsFolderPath(Assets.images.terrain.water.abcdefgh.abcdefgh0);
  ...
  Future<void> _loadWaterTiles() async => _loadAssets(
        Assets.images.terrain.water.abcdefgh.values +
            Assets.images.terrain.water.cdefg.values,
      );
